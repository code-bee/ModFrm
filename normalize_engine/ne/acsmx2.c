...
/*==================================================================
* Function	: acsmNew2
* Description	: 新建 一个自动机，参数为返初始化时调用的反注册函数，可直接为空
* Input Para	:
* Output Para	:
* Return Value:
==================================================================*/
                         ...
                         void (*neg_list_free)(void **p))
{
  ...
  if (p)
  {
	...
	p->acsmFormat            = ACF_FULL;//ACF_BANDED;		//ACF_FULL会导致重复命中。
...
* Description	: 添加一个模式串
* Input Para	: P 自动机句柄，pat 模式串内容，n模式串长度，nocase 是否关心大小写，
                offset depth negative 暂时没用。id 可在进行执行回调时，传入使用，iid没用过。
* Output Para	:
* Return Value:
==================================================================*/
int
...
*
*   Perf-Notes:
*    1) replaced ConvertCaseEx with __inline xlatcase - this improves performance 5-10%
